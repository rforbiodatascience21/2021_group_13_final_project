...
model1 <- gravier_data_aug %>% glm(outcome ~ g2E09,
                                   data = .,
...
gravier_data_wide = gravier_data_aug %>%
  select(outcome, 
         pull(gravier_data_nested_long, gene))
...
gravier_data_wide <-
  ...
  mutate(outcome = case_when(outcome == "0" ~ "good",
                             TRUE ~ "poor"))
...
plot_PCA_shares <-
  ...
  scale_x_continuous(labels = c(1,5,10,20,50,100),
...
ggsave(filename = "results/04_plot_PCA1_vs_PCS2.png", width = 16, height = 9, dpi = 72)
ggsave(filename = "results/04_plot_rotation_matrix.png", width = 16, height = 9, dpi = 72)
ggsave(filename = "results/04_plot_PCA_shares.png", width = 16, height = 9, dpi = 72)
